...
window.system_fListData.implement({
	doDraw: function(canvas){		
	    ...
	    var html =  "<div id='"+n+"_bg' style='{position: absolute; left: 0; top: 0; width: 100%; height: 100%;background:rgba(255,255,255,0);}' ></div>"+
					"<div id='"+n+"_frame' style='{border:1px #ffffff solid;background:#fff;box-shadow:0px 10px 30px #888888;position: absolute; left: 0; top: 0; width: "+this.formWidth+"; height: "+(this.formHeight + 25).toString()+";overflow:hidden;}' >" +
						"<div id = '"+n+"_header' style='position:absolute;"+
						"left: 0; top: 0; height: 25; width: "+(this.formWidth).toString()+";cursor:pointer;background:#da251d' "+
	...
	show: function(x, y){
	    ...
	    this.formRequester = this.requester.getForm();//this.getApplication()._mainForm;
		this.sg.clear();
		this.page = 1;
		this.filter = "";
		...
		if (this.serviceCallObject){
			var self = this;

			if (this.params.length == 0)
				var params = ["","", this.addFilter, this.page];
			else {
				var params = this.params;
				params[2] = ["","", this.addFilter, this.page];
			}
			this.serviceCallObject.call(this.methodName, params, function(data){
					self.sg.clear();
					self.sg.setData(data, [self.fields[0],self.fields[1]], self.fields[0]);												
					//this.sg.setData(result, true, 15);												
					if (self.withBlankRow){					    						
							self.sg.addRowValues(['-','-']);
					}
					self.getApplication().setActiveForm(self);
					//this.sg.setNoUrut((this.page - 1) * 20);
					
					self.bInfo.setCaption(self.page +"/"+ data.totalPage);
				});
		}else {
	...
	hide: function(){	
		...
		if (this.getApplication()._mainForm.activeChildForm)
			this.getApplication()._mainForm.activeChildForm.unblock();
	},
	request : function(page){
		var self = this;
		if (this.params.length == 0)
			var params = [this.e0.getText(),this.e1.getText(), this.addFilter, this.page];
		else {
			var params = this.params;
			params[2] = [this.e0.getText(),this.e1.getText(), this.addFilter, this.page];
		}
	...
	createComp: function(){
		try{		 
			...
			this.b1 = new button(this,{bound:[10,this.formHeight - 32,80,22], click:[this,"doClick"], caption:"OK", borderColor:"#79B8ED",  fontColor:"#fff"});//color:"#6AC4F1",
			this.b2 = new button(this,{bound:[100,this.formHeight - 32,80,22], click:[this,"doClick"], caption:"Cancel", borderColor:"#79B8ED", fontColor:"#fff"});
			this.ctrl = new control(this, {bound:[3,this.formHeight - 62,this.formWidth - 9, 22]});this.ctrl.getCanvas().css({background:"#da251d"});
			this.bFirst = new button(this,{bound:[10,this.formHeight - 62,20,20], click:[this,"doClick"], caption:"Cancel", borderColor:"#79B8ED", fontColor:"#fff", icon:"icon/page-first.png"});
			this.bPrev = new button(this,{bound:[34,this.formHeight - 62,20,20], click:[this,"doClick"], caption:" ", borderColor:"#79B8ED", fontColor:"#fff", icon:"icon/arrow-left.png"});
			this.bInfo = new button(this,{bound:[58,this.formHeight - 62,60,20], click:[this,"doClick"], caption:" ", borderColor:"#79B8ED", fontColor:"#fff"});
			this.bNext = new button(this,{bound:[122,this.formHeight - 62,20,20], click:[this,"doClick"], caption:" ", borderColor:"#79B8ED", fontColor:"#fff", icon:"icon/arrow-right.png"});
			this.bLast = new button(this,{bound:[146,this.formHeight - 62,20,20], click:[this,"doClick"], caption:" ", borderColor:"#79B8ED", fontColor:"#fff", icon:"icon/page-last.png"});
			this.sg.getCanvas().shadow({radius:0});
            
		}catch(e){
			systemAPI.alert("[fListData2]::init:"+e);
		}
	},
	doEditKeyPress: function(sender, charCode, buttonState){
	   if (charCode == 13){
	        ...
	        if (this.serviceCallObject != undefined){
				...
				this.filter = this.operator + "  ("+this.fields[0]+" like '"+this.e0.getText()+"%' and "+this.fields[1]+" like '"+ this.e1.getText()+"%' )";
				//console.log(this.scriptSql + this.filter +" order by "+this.orderBy);
				this.dbLib.getDataProviderPageA(this.scriptSql + this.filter +" order by "+this.orderBy, 1,this.pager);                         
