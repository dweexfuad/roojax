window.app_saku_modul_fUpdateJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku_modul_fUpdateJurnal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_modul_fUpdateJurnal";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Update Data Jurnal", 0);
		...
		this.bprint = new button(this.tab2.childPage[0], { bound: [20, 5, 80, 30],  icon:"<i class='fa fa-print' style='color:white'></i>", caption: "Print"});
		this.bprint.setColor("#00818a");
		...
		this.bprint = new button(this.tab2.childPage[1], { bound: [20, 5, 80, 30],  icon:"<i class='fa fa-print' style='color:white'></i>", caption: "Print"});
		this.bprint.setColor("#00818a");
		this.bprint.on("click", function () {
		...
		this.bprint = new button(this.tab2.childPage[2], { bound: [20, 5, 80, 30],  icon:"<i class='fa fa-print' style='color:white'></i>", caption: "Print"});
		this.bprint.setColor("#00818a");
		this.bprint.on("click", function () {
		...
		this.bprint = new button(this.tab2.childPage[3], { bound: [20, 5, 80, 30],  icon:"<i class='fa fa-print' style='color:white'></i>", caption: "Print"});
		this.bprint.setColor("#00818a");
		this.bprint.on("click", function () {
				...
				self.app.services.callServices("financial_mantis","getAksesUpdate",[self.app._userLog,self.app._lokasi], function(data){				
					if(data.induk.akses == "ADMIN"){
						...
						if(self.sg1.cells(11, row) == "-"){
							...
							self.app.services.callServices("financial_mantis","getDataUpdateJurnal",[self.sg1.cells(0, row),self.sg1.cells(10, row)], function(data){
		...
		this.bSave.on("click",function(sender){
			try{
				...
				if (self.app._periode != dataHeader.tahun +""+dataHeader.bulan){
					self.alert("Periode tidak sesuai dengan periode Open", "Transaksi dibatalkan");
					return;
				}
				...
				if (validasiPenerima) {
					...
					self.app.services.callServices("financial_mantis","isPeriodeOpen",[self.cocd.getText() , self.tahun.getText() , self.bulan.getText()], (data) => {
						...
						self.app.services.callServices("financial_mantisUpload","isBARekonSubmitted", [self.icm.getText().substr(0,11), self.cocd.getText(), periode,listAkun ] ,function(submitted){
							if (!submitted){
								self.app.services.callServices("financial_mantis","updateInputICM", [self.app._lokasi, dataHeader, dataDetail, self.baru, self.idheader_old] ,function(data){
		...
		this.bDel.on("click",function(sender){
			try{
				...
				self.confirm("Jurnal BA Rekon","Yakin Jurnal Akan di hapus?", function(result){
					...
					self.app.services.callServices("financial_mantis","delInputICM", [self.app._lokasi, dataHeader, dataDetail, self.no_jurnal_patokan_delete] ,function(data){
						if (data == "process completed") {
							system.info(self, data,"");
							self.cocd.setText("");
							self.jasa.setText("");
		...
		this.f05 = new saiLabelEdit(this.pSearch,{bound:[20,95,330,20],caption:"No Jurnal", items:["Open","Clear","Unclear","Close"], });
		...
		try{			
			...
			this.f01.setText(this.app._periode);
...
window.app_saku_modul_fUpdateJurnal.extend(window.childForm);
window.app_saku_modul_fUpdateJurnal.implement({	
	RefreshList : function(page){
		try{
			...
			self.app.services.callServices("financial_mantis","getListUpdJurnal",[self.app._lokasi], function(data){
		...
		try{			
			...
			if (sender == this.bOk){				
				...
				if (self.f1.getText() == "" && self.f2.getText() == "" && self.f3.getText() == "" && self.f4.getText() == "" && self.f5.getText() == "" && self.f6.getText() == "" && self.f7.getText() == "" ){
						...
						self.app.services.callServices("financial_mantis","cariUpdJurnal",[self.f1.getText(),self.f2.getText(),self.f3.getText(),self.f4.getText(),self.f5.getText(),self.app._lokasi,self.cek1.selected,self.cek2.selected, self.status_pembeda, self.f6.getText(), self.f7.getText()], function(data){
							self.sg1.clear(0);
								$.each(data, function(key, val){
									...
									self.sg1.addRowValues([ val.icm, val.cc, val.tp, val.year+val.month, val.kode_jasa, val.jenis_jasa, val.pekerjaan, val.currency, val.fcbp, val.status, val.idheader, val.idheader_clear, "<center><i class='fa fa-eye' style='color:red;margin-top:2px'> </i>", "<center><i class='fa fa-eye' style='color:green;margin-top:2px'> </i>" ]);
