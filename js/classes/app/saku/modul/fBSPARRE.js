window.app_saku_modul_fBSPARRE = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_modul_fBSPARRE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_modul_fBSPARRE";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Parameter Akun Laba Ditahan", 0);
		...
		this.fAkunParam = new saiCBBL(this.tab.childPage[0],{bound:[20,70,300,20],caption:"Akun RE",readOnly:true, change:[this,"doChange"]});      
		...
		this.sg1 = new saiGrid(this.tab.childPage[0],{bound:[20,175,this.tab.width-45,this.tab.height-200],
			colCount: 4,
			colTitle: ["ID","Kode Akun","Nama Akun","COCD"],
			colWidth:[[3,2,1,0],[50,200,100,120]],			
		...
		this.sg1.on("dblClick", function(col, row, id){
			if(typeof row != 'undefined'){
				...
				self.fAkunParam.setText(self.sg1.cells(1,row));
				self.fCOCD.setText(self.sg1.cells(3,row));
			}
		});

        this.fsearch.on("keyup", function(){
            self.app.services.callServices("financial_BsplMaster","getDataParamAkun",[self.app._lokasi,'RE',self.fsearch.getText()], function(data){
				// console.log(data);
...
window.app_saku_modul_fBSPARRE.extend(window.childForm);
window.app_saku_modul_fBSPARRE.implement({
	...
	doExport: function(){
		...
		this.app.services.callServices("financial_BsplMaster","xlsDataParamAkun",[self.app._lokasi,'RE',self.fsearch.getText()], function(data){
			self.hideLoading();
	...
	loadReport: function(){
		var self = this;
		self.sg1.clear(0);	
		this.app.services.callServices("financial_BsplMaster","getDataParamAkun",[this.app._lokasi,"RE",""], function(data){
			if (typeof data != 'undefined' && data != null){
                console.log(data);
				$.each(data, function(k, val){
					self.sg1.addRowValues([
	...
	loadID: function(){
		var self = this;
        self.app.services.callServices("financial_BsplMaster","getIDParamAkun",[self.app._lokasi,'RE'],function(data){  
