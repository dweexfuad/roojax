window.app_saku_modul_fBSRekAset = function(owner, pdrk)
{
	if (owner)
	{
		...
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Koreksi Aset", 99);
			...
			if (value.length > 0) {
				...
				self.myTimeExecSearch = setTimeout( function(){
					if(col == 3){
						...
						if(self.periode_today.substr(0, 4) != self.fPeriode.getText().substr(0, 4)){
							system.alert(self, "Data Margin tidak boleh berubah karena icm sudah beda tahun");
							self.sg1.setCell(5, row, self.data_rev[row].margin);													
						}else{
							var total_rev = 0;
							if(value > nilaiToFloat(self.sg1.cells(2,row))){
								system.alert(self, "Nilai Revenue BSPL " + floatToNilai(value) + " tidak boleh lebih besar dari nilai Revenue ICM " + floatToNilai(self.sg1.cells(2,row)));
			...
			if (value.length > 0) {
				...
				self.myTimeExecSearch = setTimeout( function(){
					if (col == 0) {
						...
						if(self.periode_today.substr(0, 4) != self.fPeriode.getText().substr(0, 4)){
							system.alert(self, "Data cogs tidak boleh berubah karena icm sudah beda tahun");
							self.sg2.setCell(2, row, self.data_cogs[row].nilai);													
						}else{
							var total_cogs = 0;
							self.reverse_nilai_global = 0;
		...
		this.sg3 = new saiGrid(this.tab.childPage[1],{bound:[10,355,this.tab.width-40,this.tab.height-210],
			...
			columnReadOnly:[true, [22,21,20,19,18,17,16,15,14,12,11,5,1],[13,10,9,8,7,6,4,3,2]],
			rowperpage:1000, autoPaging:true,pasteEnable: true, pager:[this,"doPager3"]});
		var self = this;
				...
				self.myTimeExecSearch = setTimeout( function(){
					if (col == 2) {
						if (typeof self.data_aset[row] != 'undefined' && self.data_aset[row] != null){
							system.alert(self, "Akun Aset tidak boleh berubah");	
							self.sg3.setCell(2, row, self.data_aset[row].orig_acct);													
						}else{
						...
						if(self.akunADK.indexOf(self.sg3.cells(2,row)) != -1 || self.sg3.cells(2,row).match(/1319.*/) || self.sg3.cells(2,row).match(/1320.*/)){  
							self.sg3.setCell(7, row, '');
							...
							if(alert_error != ''){
								...
								self.sg3.setCell(7, row, thn_icm + '/' + bln_icm + '/' + new Date(thn_icm, bln_icm, 0).getDate());
							}
						}
					}else if(col == 8){
						if (self.bacc_icm.includes(value) != true){
						...
						if(self.akunADK.indexOf(self.sg3.cells(2,row)) != -1 || self.sg3.cells(2,row).match(/1319.*/) || self.sg3.cells(2,row).match(/1320.*/)){  
							...
							var thn_tgl_perolehan = self.sg3.cells(7,row).substr(0, 4);						
							var bln_tgl_perolehan = self.sg3.cells(7,row).substr(5, 2);
							...
							if(thn_periode_susut != thn_tgl_perolehan){
								...
								self.sg3.setCell(10, row, yyyy + bln_icm);					
							}
							...
							if(periode_susut < periode_icm){
								alert_error = alert_error + "Periode susut (" + periode_susut + ") tidak boleh lebih kecil daripada periode aset (" + periode_icm + ") <br/>";
								self.sg3.setCell(10, row, yyyy + bln_icm);
							}
						...
						if(self.periode_today.substr(0, 4) != self.fPeriode.getText().substr(0, 4)){
							system.alert(self, "Data nilai aset tidak boleh berubah karena icm sudah beda tahun");
							self.sg3.setCell(13, row, self.data_aset[row].nilai);													
						}else{
