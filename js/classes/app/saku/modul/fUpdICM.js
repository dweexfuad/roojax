window.app_saku_modul_fUpdICM = function(owner,menu)
{
	if (owner)
	{
		try{
			window.app_saku_modul_fUpdICM.prototype.parent.constructor.call(this,owner);
			this.className  = "app_saku_modul_fUpdICM";
			this.itemsValue = new arrayList();		
			this.maximize();	
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Update ICM ", 0);

			uses("column;pageControl;saiEdit;datePicker;saiGrid;saiCBBL;childPage;panel;sgNavigator;column", true);			
			var self=this;
			self.notif="-";
			if(menu == undefined){
				menu = this.app._menu;
			...
			this.bulan = new saiCB(this,{bound:[230,70,50,20],caption:"Bulan", labelWidth:0, visible:true, items:["01","02","03","04","05","06","07","08","09","10","11","12"], change:[this,"doChange"]});
			this.bulan.setItemHeight(12);
			this.tahun = new saiCB(this,{bound:[20,70,200,20],caption:"Tahun", visible:true, items:[this.app._periode.substr(0,4)], change:[this,"doChange"]});
			...
			this.bDel.on("click",function(sender){
				try{
					...
					self.confirm("Data BA Rekon","Yakin Data Akan di hapus?", function(result){
						self.app.services.callServices("financial_mantis","UpdICM", [dataHeader, self.app._lokasi, self.tahun.getText() + self.bulan.getText()] ,function(data){
...
window.app_saku_modul_fUpdICM.extend(window.childForm);
window.app_saku_modul_fUpdICM.implement({	
