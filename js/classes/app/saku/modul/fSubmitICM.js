window.app_saku_modul_fSubmitICM = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_modul_fSubmitICM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_modul_fSubmitICM";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Submit BA Rekon ICM to Review", 0);
		...
		this.bOk.on("click", () => {
			...
			let status = "USER";//this.f4.getText();
		...
		//View BA
		var dokViewer = new control(this.tab.childPage[1], {bound:[0,30, this.tab.width-2, this.tab.height-85 ]});	
		var luas = this.width;	
		...
		this.eMemo = new saiMemo(this.tab.childPage[1],{bound:[20,this.tab.height-55,800,50], caption:"Catatan"});
		
		this.bSubmit = new button(this.tab.childPage[1], {bound:[840,this.tab.height-55,100, 30], caption:"Submit", icon:"<i class='fa fa-send' style='color:white'/>"});
...
window.app_saku_modul_fSubmitICM.extend(window.childForm);
window.app_saku_modul_fSubmitICM.implement({
