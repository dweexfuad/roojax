window.app_saku_report_fBSTax = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_report_fBSTax.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_report_fBSTax";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Report Jurnal Pajak", 99);
		...
		// this.fJenis = new saiCB(this.pnl1,{bound:[327, 23, 170, 20],labelWidth:80, caption:"Jenis Jurnal", items:["ALL","MARGIN","COGS","PAJAK"]});
...
window.app_saku_report_fBSTax.extend(window.childForm);
window.app_saku_report_fBSTax.implement({
	...
	doClick: function(sender){
		if(sender == this.bGenerateJurnal){
			...
			// if(this.fTglJurnal.getText() != '' || this.fTglJurnal2.getText() != '' || this.fICM.getText() != '' || this.fCOCD.getText() != '' || this.fTP.getText() != ''){
				this.loadReport();
			// }else{
	...
	doExport: function(){
		...
		this.app.services.callServices("financial_BsplMaster","xlsJurnalByJenis",[self.app._lokasi,'','TAX',filter], function(data){
			self.hideLoading();
			window.open("./server/reportDownloader.php?f="+data+"&n="+"JURNAL_PAJAK.xlsx");
		}, function(){
			self.hideLoading();
		});	
	},
	...
	loadReport: function(){
		...
		this.app.services.callServices("financial_BsplMaster","getJurnalByJenis",[this.app._lokasi,'','TAX',filter], function(data){
