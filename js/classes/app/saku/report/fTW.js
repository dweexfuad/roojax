window.app_saku_report_fTW = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_report_fTW.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_report_fTW";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Laporan Summary BA(F19)", 99);
...
window.app_saku_report_fTW.extend(window.childForm);
window.app_saku_report_fTW.implement({
	...
	getSummaryBA: function(lokasi = null, icm = null, akun = null, perio = null, tp = null, status = null, jasa = null){
		...
		this.app.services.callServices("financial_mantisRepTW", "getListEliminasi", [this.tmpFilter], (data) => {
			...
			for(var key in data){
				...
				sa_loc += Math.round(parseFloat(line.loc_sawal));
				mu_loc += Math.round(parseFloat(line.loc_amount));
				sak_loc += Math.round(parseFloat(line.loc_sakhir));
				adj_loc += Math.round(parseFloat(line.loc_amount_adj));
				sakadj_loc += Math.round(parseFloat(line.loc_sakhir_adj));
