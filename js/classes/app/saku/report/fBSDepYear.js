window.app_saku_report_fJurDepYear = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_report_fJurDepYear.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_report_fJurDepYear";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Report Jurnal Depresiasi Current Year Oy", 99);
		...
		this.fYear = new saiLabelEdit(this.pnl1,{bound:[327, 46, 170, 20],labelWidth:80,caption:"Tahun",placeHolder:"YYYY"});		
...
window.app_saku_report_fJurDepYear.extend(window.childForm);
window.app_saku_report_fJurDepYear.implement({
	...
	loadReport: function(){
		...
		var filter = {
			...
			tp : self.fTP.getText(),
			jenis_jurnal : self.fJenis.getText(),
			year : self.fYear.getText(),
		};
		this.app.services.callServices("financial_BsplMaster","getJurnalByJenisYtd",[this.app._lokasi,'','CURRENT_PORTION',filter], function(data){
