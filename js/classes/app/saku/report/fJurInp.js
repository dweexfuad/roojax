window.app_saku_report_fJurInp = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_report_fJurInp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_report_fJurInp";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Report Jurnal Current", 99);
		...
		this.fJenis = new saiCB(this.pnl1,{bound:[327, 23, 170, 20],labelWidth:80, caption:"Jenis Jurnal", items:["ALL","MARGIN","COGS"]});
...
window.app_saku_report_fJurInp.extend(window.childForm);
window.app_saku_report_fJurInp.implement({
	...
	doExport: function(){
		...
		this.app.services.callServices("financial_BsplMaster","xlsJurnalByJenis",[self.app._lokasi,'','CURRENT',filter], function(data){
			self.hideLoading();
			window.open("./server/reportDownloader.php?f="+data+"&n="+"JURNAL_CURRENT.xlsx");
		}, function(){
			self.hideLoading();
		});	
	},
	...
	loadReport: function(){
		...
		this.app.services.callServices("financial_BsplMaster","getJurnalByJenis",[this.app._lokasi,'','CURRENT',filter], function(data){
