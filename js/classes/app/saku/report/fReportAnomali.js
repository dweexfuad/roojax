window.app_saku_report_fReportAnomali = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_report_fReportAnomali.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_report_fReportAnomali";
		this.itemsValue = new arrayList();		
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Laporan Anomali", 99);
		...
		this.summaryBA = new saiGrid(this,{bound:[10,120,this.width-25,this.height-125],
			colCount: 7, headerHeight: 65, pasteEnable: false,
			colTitle: [
				{
...
window.app_saku_report_fReportAnomali.extend(window.childForm);
window.app_saku_report_fReportAnomali.implement({
	...
	exportEliminasi: function(filter){
		this.showLoading("Please wait...");
		console.log("masuk sini...");
		this.app.services.callServices("financial_mantisTB", "downloadAnomali", [this.eCocd.getText(), this.ePeriode.getText()], (filename) => {
			this.hideLoading();
			window.open("./server/reportDownloader.php?f="+filename+"&n=Anomali.xlsx");
		}, (err) => {
			this.hideLoading();
		});
	},
	getSummaryBA: function(){
		console.log("call getSummaryBA");
		this.showLoading("Please wait...");
		this.app.services.callServices("financial_mantisTB", "getReportAnomali", [this.eCocd.getText(), this.ePeriode.getText()], (data) => {
			...
			for(var key in data){
				...
				this.summaryBA.addRowValues([
					line.kode_akun, line.akun_tlkm,line.description,  line.amount, line.loc_sakhir, line.amount - line.loc_sakhir, '-'
				]);
