window.app_saku_report_fElim = function(owner, pdrk)
{
	if (owner)
	{
		window.app_saku_report_fElim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_report_fElim";
...
window.app_saku_report_fElim.extend(window.childForm);
window.app_saku_report_fElim.implement({
	...
	exportEliminasi: function(filter){
		...
		this.app.services.callServices("financial_mantisRepTW", "exportReportEliminasi2", [this.tmpFilter], (filename) => {
			this.hideLoading();
	...
	getSummaryBA: function(lokasi = null, icm = null, akun = null, perio = null, tp = null, status = null, jasa = null){
		...
		this.showLoading("Please wait...");
		this.app.services.callServices("financial_mantisRepTW", "getReportEliminasi2", [this.tmpFilter], (data) => {
			...
			for(var key in data){
				...
				/*
				{"icm":"19031010033","year":"2019","month":"12","cc":"1424","tp":"1008","fcbp":" ","bacc":"-","batp":"-","kode_akun":"11602007"
					,"currency":"IDR","kode_jasa":"101","jenis_jasa":"Jasa Fee Voucher Pulsa / RITNAS"
					,"gl_acct":"11602007","urutan":"6","klp":"3","jenis_rekon":"BDD"
					,"doc_sawal":"360114349732"
					,"loc_sawal":"360114349732"
					,"doc_amount":"177233120796"
					,"loc_amount":"177233120796"
					,"doc_sakhir":"537347470528"
					,"loc_sakhir":"537347470528"
					,"loc_sakhir_adj":"537347470528"
					,"doc_sakhir_ad":"537347470528"
					,"elim_reg":"-279456557820.79551164027599975715974389","elim_bspl":"0","balance":0,"ba_rekon":"BA-3","status_ba":"PELAKU REKON","status":"NON BSPL","mapcc":"1424","maptp":"1008","cfu":"CFU4","cfu_lawan":"CFU1","doc_amount_adj":"0","loc_amount_adj":"0","description":"eVoucher TelkomselFINNET - FINNET","short_text":"eVoucher TelkomselFINNET - FINNET"}
				*/
				if (line.currency == "USD"){
					sausd_doc += Math.round(parseFloat(line.doc_sawal));
					muusd_doc += Math.round(parseFloat(line.doc_amount));
					sakusd_doc += Math.round(parseFloat(line.doc_sakhir));
					adjusd_doc += Math.round(parseFloat(line.doc_amount_adj));
					sakadjusd_doc += Math.round(parseFloat(line.doc_sakhir_adj));
				}else{
					sa_doc += Math.round(parseFloat(line.doc_sawal));
					mu_doc += Math.round(parseFloat(line.doc_amount));
					sak_doc += Math.round(parseFloat(line.doc_sakhir));
					adj_doc += Math.round(parseFloat(line.doc_amount_adj));
					sakadj_doc += Math.round(parseFloat(line.doc_sakhir_adj));
				}
