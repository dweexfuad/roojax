...
//* 			SAI, PT											
//***********************************************************************************************
window.control_MenuForm = function(owner,options){
    if (owner){
		this.className = "control_MenuForm";
		window.control_MenuForm.prototype.parent.constructor.call(this, owner);        
		this.setWidth(100);
		this.setHeight(30);
...
window.control_MenuForm.extend(window.control_commonForm);
window.MenuForm = window.control_MenuForm;
window.control_MenuForm.implement({
	...
	popUp: function(x, y, pointMode){
		try{
		    if (this.parentForm == undefined)
		    {
		        ...
		        if (!(form instanceof control_MenuForm))
	...
	doDeactivate: function(){
		window.control_MenuForm.prototype.parent.doDeactivate.call(this);
	    var app = this.getApplication();
	...
	setParentForm: function(data){
		if (data instanceof control_MenuForm)
