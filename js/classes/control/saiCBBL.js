...
window.control_saiCBBL.implement({
	...
	setServices : function(serviceCallObject, methodName, params, fields, addFilter, caption, checkItem){
	   ...
	   try{//!this.needCheckItem || 	
		    ...
		    if (this.serviceCallObject != undefined){
				...
				if (this.params.length == 0){
					params = [this.getText(), "", this.addFilter, 0];
				}else {
					params[2] = [this.getText(),this.getText(), this.addFilter,0];
				}
				...
				if (this.getText() != ""){
					this.serviceCallObject.call(this.methodName, params , function(data){
						if (data && data.rs.rows.length > 0){
							...
							if (data.rs.rows[0][self.fields[0]] != self.getText() && self.getText() != "" ){
								self.setText(data.rs.rows[0][self.fields[0]]);
							}
							self.setRightLabelCaption(data.rs.rows[0][self.fields[1]]);
							
						}else if (self.needCheckItem){
