...
window.control_saiTreeGridNode.implement({
	...
	setData: function(data, fields, rowIndex, id){
		   ...
		   // var col = sg.columns.get(i);
			...
			var node = $("<div id='"+id+"'></div>");
				...
				if (sg.columnFormat.get(i) == cfNilai){
					...
					node.html("<div  id='"+id+"_edit' style='white-space:nowrap;position:absolute;left:3px;top:2px;width:"+(sg.columnWidth.get(i) - 6)+"px;height:"+(sg.rowHeight - 6)+"px'>"+floatToNilai(data[fields[i]])+"</div>");
					...
					node.html("<div id='"+id+"_edit' style='white-space:nowrap;position:absolute;left:3px;top:2px;width:"+(sg.columnWidth.get(i) - 6)+"px;height:"+(sg.rowHeight - 6)+"px'>"+data[fields[i]]+"</div>");
				...
				node.bind("click",{gridId:sg.resourceId, col:i, row:this.rowIndex, resourceId:this.resourceId}, function(event){
					try{
						...
						$$$(event.data.gridId).onClick.call($$$(event.data.gridId), event.data.col, event.data.row);
					}catch(e){
						error_log(e);
					}
				});
		...
		this.getCanvas().css({background:sg.levelColor[this.level], width:left + 38});
