...
window.control_saiTreeGridNode.implement({
	doDraw : function(canvas){
	    ...
	    var html =  "<div style='{cursor: pointer; position : relative; left: 0; top: 0;width: 100%; height: "+tree.rowHeight+";' " +
	                   ...
	                   //
	                    ...
	                    "<div id='" + n + "_frame' style='{position : absolute; left: "+leftCaption+"; top: 0; width: auto; height: 100%;background: url(images/transparent.png) top left;}' " +
	                        ...
	                        "></div>" +
                        "<div style='top:100%;width:100%;height:1px;background:#999'></div>"+
	                "</div>" +
			...
			var node = $("<div id='"+id+"'></div>");
				...
				node.bind("click",{gridId:sg.resourceId, col:i, row:this.rowIndex, resourceId:this.resourceId}, function(event){
					try{
						...
						sg.emit("click", event.data.col, event.data.row, event);
				...
				node.bind("dblclick", {gridId:sg.resourceId, col:i, row:len, resourceId: this.resourceId}, function(event){
					...
					sg.emit("dblclick", event.data.col, event.data.row, event);
				});
				node.bind("mousedown",{gridId:sg.resourceId, col:i, row:len, resourceId: this.resourceId}, function(event){
					if (event.button == 2){
						event.stopPropagation();
						sg.emit("cellmousedown",  event.data.col, event.data.row, $$$(event.data.resourceId),  event);
						return false;
					}
					return true;
				});
