...
window.control_saiTreeGrid = function(owner, options){	
    if (owner){
		try{			
			...
			this.levelColor = ["#BCFAFF","#C6FFE4","#E6FFCF","#FFE5D0","#FFB6B3","#FCBCFF","#B7C0FF","#84D1FF","#64FFB3","#E3FF81"];
	...
	doDraw: function(canvas){
		...
		var w = this.width - this.widthColNo;
	   	var  html =  "<div id='" + n + "_frame' style='{position : relative; left: 0; top: 0; width: 100%; height: 100%;overflow: hidden;}' onscroll='$$$("+this.resourceId+").doScrollFrame(event)'>" +                	
						...
						"</div>"+
						"<div id='"+n+"_header' style='position:absolute;overflow:hidden;left: 38px;top: 0; width:auto; height:"+this.headerHeight+"px;border:1px solid #transparent'"+
							" onmousemove ='$$$("+this.resourceId+").headerMove(event)' "+	
							" onmouseup ='$$$("+this.resourceId+").headerUp(event)' "+	
						"></div>"+
						"<div id='"+n+"_freeze' style='{background:#fff;position:relative;left: 0;top: "+this.headerHeight+"; width: "+w+"px; height: "+h+"px; overflow: hidden;}' onscroll='$$$("+this.resourceId+").doScroll(event)' >"+
						"</div>"+
