	...
	doDraw: function(canvas){
		try{
			...
			if (this.owner instanceof column && this.columnIndex == 0)
				var node = $("<div id='"+n+"_colHead' style='position:absolute;height:100%;top:0;left:"+left+";width:"+this.width+";background:#EEEEEE;color:#007AFF'></div>");									
			else 
				var node = $("<div id='"+n+"_colHead' style='position:absolute;height:100%;top:0;left:"+left+";width:"+this.width+";background:#EEEEEE;color:#007AFF;border-right:1px solid #007AFF;border-left:1px solid #007AFF'></div>");									
			...
			var html =  "<div id='"+ n +"_sort' style='position:absolute;top:2px;left:2px;display:none;width:11;height:10'></div>"+
						...
						">"+this.title+"</div>"+
						"<div id='"+ n +"_header' style='position:absolute;top:"+this.owner.headerHeight+"px;left:0px;width:100%;height:100%;overflow:hidden'></div>";
	...
	setTitle: function(data, colIndex){
	    if (typeof (data) == "string"){
		  ...
		  $("#"+this.getFullId()+"_column").html("<table height='100%' width='100%' border=0 style='font-size:12;color:#007AFF'><tr><td valign='middle' align='center'> "+this.title+"</td></tr></table>" );
		    ...
		    $("#"+this.getFullId()+"_header").css({borderTop:"1px solid #ff9900",borderBottom:"1px solid #ff9900", top : this.owner.headerHeight +"px"});
            ...
            if (data.column){
                try{
                	if (data.columnAlign){
                		...
                		$("#"+this.getFullId()+"_header").css({borderTop:"1px solid #007AFF",borderBottom:"1px solid #ff9900", top:h+"px"});
	                    $("#"+this.getFullId()+"_column").css({height:h+"px"});
	                    this.headerHeight = 20;
                	}else{
	                    $("#"+this.getFullId()+"_header").css({borderTop:"1px solid #007AFF",borderBottom:"1px solid #ff9900", top:"20px"});
	...
	refreshCells: function(freeze){
		try{	
            ...
            for (var i = 0; i < grid.getRowCount();i++){	
				...
				else var left = this.headerCanvas.offset().left - grid.headerCanvas.offset().left ;
