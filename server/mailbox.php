...
$timeout          = 90;                                                        //    Default timeout before givingup on a network operation.
...
//-----------------------------------------------------------------------------
   //echo "<h2>Starting Program</h2><br>";
        ...
        $fp = connect ($server, $port = 995);
	if (!$fp){	
		echo "Failed to connect server...<br>";
		return false;	
	}	
    ...
    if( (!$Count) or ($Count == -1) )
    {
        echo ("Check for new messages.... No Messages<br>");
        exit;
    }// end if
    ...
    if ($Count < 1)
    {
        ...
        echo ("Check for new messages... Count Messages<br>");
