...
function connect ($server, $port)
    {
        ...
        $e_server = $server;
	echo "connecting..... $e_server($port)<br>";
        if(!$fp = fsockopen($e_server, $port, &$errno, &$errstr))
        {
            echo "POP3 connect: Error [$errno] [$errstr]<br>";
            return false;
        }
        ...
        if($RFC1939)
        {
            echo  "POP3: premature NOOP OK, NOT an RFC 1939 Compliant server<br>";
            quit($fp);
            return false;
        }
	echo "connected to..... $e_server($port)<br>";
        return $fp;
    }// end function
...
function user ($user, $fp)
    {
        ...
        if(empty($user))
        {
            echo "POP3 user: no user id submitted<br>";
            return false;
        }
...
function pass ($pass, $fp)
    {
        ...
        if(empty($pass))
        {
            echo "POP3 pass: no password submitted<br>";
            return false;
        }
        ...
        if(!is_ok($reply))
        {
            echo "POP3 pass: authentication failed [$reply]<br>";
            quit($fp);
            return false;
        }
        //    Auth successful.
        ...
        if(!$RFC1939)
        {
            echo "POP3 pass: NOOP failed. Server not RFC 1939 compliant<br>";
            quit($fp);
            return false;
        }
        return $count;
...
function login ($login = "", $pass = "", $fp)
	...
	echo "login as $login...<br>";
        if(!user($login, $fp))
        {
            //    Preserve the error generated by user()
	    echo "error login... $login<br>";
            return false;
        }
...
function send_cmd ( $cmd, $fp )
	...
	error_log($cmd);
        ...
        if(!isset($fp))
        {
            echo "POP3 send_cmd: No connection to server<br>";
            return false;
        }

        if(empty($cmd))
        {
            echo "POP3 send_cmd: Empty command string<br>";
            return "";
	}
        $buffer = $buffer;
        update_timer();
