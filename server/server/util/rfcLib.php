...
class server_util_rfcLib  extends server_BasicObject
{
	...
	function setConfig($host, $sysnr, $client, $codepage){
		$this->host = $host;
		$this->sysnr = $sysnr;
		$this->client = $client;
	...
	function login($usr, $pwd){
		...
		try{
            ...
            //error_log($this->host .":".$this->sysnr.":".$this->client .":".$usr .":".$pwd);
            $login = array (
				"ASHOST"=>$this->host,
				"SYSNR"=>$this->sysnr,
				"CLIENT"=>$this->client,
           ...
           return saprfc_open ($login );
			...
			error_log($e->getMessage() ." :" .saprfc_error());
			echo($e->getMessage());
			return 0;
		}
	}
				...
				if (isset($sapExpTable)){
					foreach ($sapExpTable->getArray() as $expValue){
						...
						for ($i=1;$i<=$rows;$i++){
							...
							$rowFile = "";
							foreach ($rowValue as $key => $val){
                                if (trim($key) == "AMOUNT" || $key == "VAL_LOCCURR"){
                                    if (strpos($val,'-') > -1){
                                        $val = -1 * floatval($val);
    ...
    function callRFCToTable($login, $sapFunc, $sapImp,  $sapExpTable = null, $sapImpTable = null, $sapExp = null, $db, $table, $field){
		try{
			unlink($file);
			if (!isset($rfc)) $rfc = $this->login($login->get("user"),$login->get("passwd"));
			if ($rfc){
				...
				if (isset($sapExpTable)){
					foreach ($sapExpTable->getArray() as $expValue){
						$rows = saprfc_table_rows ($fce,$expValue);
						$first = true;
                        $sqlTable = "insert into " . $table . "($field)values";
						for ($i=1;$i<=$rows;$i++){
							$rowValue = saprfc_table_read ($fce,$expValue,$i);
							$sqlValue = "";
							foreach ($rowValue as $key => $val){
