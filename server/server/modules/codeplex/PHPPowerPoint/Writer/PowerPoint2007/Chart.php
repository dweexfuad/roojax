...
{	
			...
			$objWriter->startElement('c:chart');
				...
				if (!$chart->getView2D()){
					...
					$objWriter->startElement('c:view3D');
						...
						// c:hPercent
						if (!$chart->getView3D()->getAutoScale()){
	...
	public function writeSpreadsheet(PHPPowerPoint $presentation, $chart, $tempName) {
		...
		// Write series
		$seriesIndex = 0;
		if ($chart->getPlotArea()->isMultiType()){
			$types = $chart->getPlotArea()->getTypes();
			foreach($types as $type){
				foreach ($type->getData() as $series) {
					// Title
	...
	protected function _writePlotArea(PHPPowerPoint_Shared_XMLWriter $objWriter, PHPPowerPoint_Shape_Chart_PlotArea $subject, PHPPowerPoint_Shape_Chart $chart) {
		...
		$objWriter->startElement('c:plotArea');
			...
			// Write chart
			if ($subject->isMultiType()){
				$types = $subject->getTypes();
				$seriesIndex = 0 ;
				foreach ($types as $key => $chartType) {
					if ($chartType instanceof PHPPowerPoint_Shape_Chart_Type_Bar3D) {
						...
						$this->_writeTypeLine($objWriter, $chartType, $chart->getIncludeSpreadsheet(),$seriesIndex);
						...
						$this->_writeTypeBar($objWriter, $chartType, $chart->getIncludeSpreadsheet(),$seriesIndex);
					} else {
						throw new Exception('The chart type provided could not be rendered.');
					}	
					$seriesIndex += count($chartType->getData());
				}
				...
				if ($chartType instanceof PHPPowerPoint_Shape_Chart_Type_Bar3D) {
					...
					$this->_writeTypeBar($objWriter, $chartType, $chart->getIncludeSpreadsheet());
				} else {
					throw new Exception('The chart type provided could not be rendered.');
				}	
			}
			...
			if ($subject->hasDataTable()){
				// c:dTable
				$objWriter->startElement('c:dTable');
					$dataTable = $subject->getDataTable();
					$objWriter->startElement('c:showHorzBorder');
					$objWriter->writeAttribute('val', ($dataTable->getShowHorzBorder() ? 1 : 0));
					$objWriter->endElement();
					$objWriter->startElement('c:showVertBorder');
					$objWriter->writeAttribute('val', ($dataTable->getShowVertBorder() ? 1 : 0));
					$objWriter->endElement();
	...
	protected function _writeLegend(PHPPowerPoint_Shared_XMLWriter $objWriter, PHPPowerPoint_Shape_Chart_Legend $subject) {
		...
		$objWriter->startElement('c:legend');
			...
			$objWriter->startElement('c:spPr');
				...
				// Fill
				if ( $subject->getFill()->getFillType() != PHPPowerPoint_Style_Fill::FILL_NONE)
			...
			foreach ($subject->getData() as $series) {
				...
				$objWriter->startElement('c:ser');
					...
					$objWriter->startElement('c:dLbls');
						...
						// c:showVal
						$this->_writeElementWithValAttribute($objWriter, 'c:showLegendKey', $series->getShowLegendKey() ? '1' : '0');
						// c:showVal
	...
	protected function _writeTypeBar(PHPPowerPoint_Shared_XMLWriter $objWriter, PHPPowerPoint_Shape_Chart_Type_Bar $subject, $includeSheet = false, $seriesIndex = 0) {
		// c:bar3DChart
		$objWriter->startElement('c:barChart');
			...
			// c:grouping
			$objWriter->startElement('c:grouping');
			$objWriter->writeAttribute('val', $subject->getGrouping());
			$objWriter->endElement();
			...
			foreach ($subject->getData() as $series) {
				...
				$objWriter->startElement('c:ser');
					...
					// c:dLbls
					error_log($series->getTitle() .":".$series->getShowDataLabel());
					if ($series->getShowDataLabel() === true){
						$objWriter->startElement('c:dLbls');
					...
					// c:spPr
					$objWriter->startElement('c:spPr');
						if ($series->getFill()->getFillType() == PHPPowerPoint_Style_Fill::FILL_NONE)
			...
			$objWriter->endElement();
			// modify for stacked
			if ($subject->getGrouping() == "stacked"){
	 ...
	 * @throws 	Exception
	 */
	protected function _writeTypeLine(PHPPowerPoint_Shared_XMLWriter $objWriter, PHPPowerPoint_Shape_Chart_Type_Line $subject, $includeSheet = false, $seriesIndex = 0) {
		// c:lineChart
