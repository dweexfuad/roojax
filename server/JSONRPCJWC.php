...
class JSONRPC
{
	...
	function getTokens($sessionId, $userid){
		// $unixTime = new DateTime();
		error_log("Session Log $sessionId");
		$unixTime = time();
		$notBefore  = $unixTime;             //Adding 10 seconds
    	$expire     = $notBefore + (60 * 60 * 4);            // Adding 60 seconds
		$token = array("uid" => $userid
						, "iat" => $unixTime
						, 'nbf'  => $notBefore        // Not before
						, 'exp'  => $expire);           // Expire);
		$jwt = JWT::encode($token, $sessionId,'HS512');
		return $jwt;
	}
	function authToken($jwt, $secretKey){
		if ($jwt) {
			...
			try {
				 ...
				 */
				// $secretKey = $this->getSessionObject($objId);
				error_log("Decode $jwt");
				$token = JWT::decode($jwt, $secretKey, array('HS512'));
				if ($token->exp < time()) {
					error_log("Enter Here time");
					return false;
				} else {
					return $token;
				}
			...
			if ($message != null)
			{
				...
				else
				{
					...
					if ($message->method == "login") {
						...
						// error_log("Token 2 $token ");
						// error_log("Token Session " . $message->session);
						$auth = $this->authToken($token, $message->session);
						if ($auth === false){
							$this->sendError(-32604, "Invalid Bad Request. ", $message->id);
							return;
						}
					}
					if (is_callable($handlerFunc))
					{
						try
						{
							...
							if (($message->method == "login" && $result["type"] != 1)|| ($message->method == "callServices" && $param[1] == "login" && $result["type"] != 1)) {
								$token = $this->getTokens($result["session"],  $param[2][0]);
								$result["token"] = $token;
